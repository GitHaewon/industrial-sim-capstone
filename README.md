...
- **실제 PLC/실물 장비 연동 없음**: 이 프로젝트는 순수 시뮬레이션이며, 실제 산업 현장의 PLC나 물리적 컨베이어/로봇팔과의 연동은 검증되지 않았습니다.
- **AMCL 위치 추정 검증 로직의 시뮬레이션 의존성**: `amr_controller.py`의 AMCL 위치 검증은 Gazebo 그라운드트루스 토픽과 비교하는 방식으로 구현되어 있어, 그라운드트루스를 제공하지 않는 실제 로봇 환경에는 그대로 적용할 수 없습니다. 실기 이전 시 별도의 위치 추정 신뢰도 검증 방식이 필요합니다.
- **대시보드 기능 확장 여지**: 현재 대시보드는 읽기 전용 모니터링만 제공하며, 시연 파라미터(시드, 재시도 횟수 등)를 대시보드에서 직접 제어하는 기능은 없습니다.
- **배포 안정성**: 단일 PC, 단일 세션 실행을 전제로 설계되어 있으며, 다중 사용자 동시 접속이나 장시간 무중단 운영에 대한 안정성은 별도로 검증되지 않았습니다. — 확인 필요.
- **팀원별 세부 기여 내역**: 코드 저장소의 커밋 이력만으로는 팀원별 세부 작업 범위를 완전히 특정하기 어렵습니다. — 추가 작성 필요.
